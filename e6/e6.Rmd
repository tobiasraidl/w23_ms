...
```{r}
biplot(pca_hubert_h)
points(pca_hubert_p$scores, col="green")
```
...
```{r}
fa <- factanal(scale(select(df, -class)),factors=8, scores="regression")
```
...
The uniquenesses are the error term $e$. There is one uniqueness value per variable. The higher it is the less variance of it is explained by the factors. 
